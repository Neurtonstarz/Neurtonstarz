...
import json
import requests
from requests.packages.urllib3.exceptions import InsecureRequestWarningrequest.packages.urllib3.disable_warnings(InsecureRequestWarning)
...
BASE_URL_STR = 'htts:{}:8443/'.format(VMANAGE_IP)
...
# Estabilish a new session and connect to Cisco vManage
...
print('{0:20s}{1:1}{2:12s}{3:1}{4:36s}{5:1}{6:16s}{7:1}{8:7s}'\
    .format(ITEM['host-name'], "|", "Device Model", "|", "Device ID", \
        "|", "System IP", "|" "Site ID"))
print('-'*105)
...
# URL for device tempalte API resource
...
for ITEM in TEMPLATE_ITEMS:
    print('{0:20s}{1:1}{2:12s}{3:1}{4:36s}{5:1}{6:16s}{7:1}{8:7s}'\
        .format(ITEM['templateName'], "|", ITEM['deviceType'], \
            ITEM['templateId'], "|", ITEM['deviceAttached'], "|", \
